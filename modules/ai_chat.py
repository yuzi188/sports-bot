"""AI 客服聊天模組 V17 - 三種運動 AI 分析整合版
新增：
  1. INTENT_SYSTEM_PROMPT 加入 football_analyze / baseball_analyze / basketball_analyze 動作
  2. 修復多語言邏輯：非 zh_tw 語言時，FAQ 中文回覆也套用語言指示
  3. 修復個資偵測：帳號關鍵字觸發 FAQ 時不再重複附加引導語
  4. 改善錯誤處理：意圖判斷失敗時回傳更精確的 fallback
"""
...
9. 「棒球分析」「今日棒球」「MLB分析」「棒球預測」= baseball_analyze
...
def _check_faq(user_message: str, lang: str = "zh_tw") -> str | None:
    ...
    修復 V17：非 zh_tw 語言時，中文 FAQ 回覆末尾替換為對應語言的引導語。
    """
...
def get_ai_response(user_id: int, user_message: str, user_lang: str = "zh_tw") -> str:
    ...
    try:
        ...
        # ── Step 3：選擇性附加客服引導語 ──
        # 只在 AI 回覆中未包含引導語，且用戶訊息明確涉及帳號/金流問題時才附加
        # 修復：縮小觸發範圍，避免過度附加
...
def should_use_bot_function(user_id: int, user_message: str) -> dict:
    ...
    V17 更新：加入 football_analyze / baseball_analyze / basketball_analyze 動作
