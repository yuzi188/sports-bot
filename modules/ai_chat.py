"""AI 客服聊天模組 V16 - 語言切換修復版
新增：
  1. get_ai_response 加入 user_lang 參數，動態調整 system prompt 語言指示
  2. FAQ 知識庫加入英文版本（en 語言時回傳英文回覆）
  3. 超出範圍回覆支援多語言
"""
...
SYSTEM_PROMPT = f"""你是「世界體育數據室」的 AI 客服助理，同時負責體育資訊查詢與平台客服服務。
...
超出範圍：
  「老闆您好，關於您諮詢的問題已經超出了客服的服務範圍，非常抱歉不能為您解答，{_CS_GUIDE}」
...
_FAQ_DB = [
    ...
    (
        ["usdt", "USDT", "U幣", "u幣", "U站", "u站", "香港站", "儲值U", "儲值u", "加密貨幣", "泰達幣", "虛擬貨幣"],
    ...
    (
        ["提現", "出金", "提款", "領錢", "提領", "出款"],
        f"老闆您好，關於提現問題，請提供您的會員帳號以便客服為您協助。\n\n{_CS_GUIDE}",
    ),
    # ── 系統維護 ──
    (
    ...
    for keywords, reply_text in _FAQ_DB:
        for kw in keywords:
            if kw.lower() in msg_lower:
                ...
                # 如果是英文模式，在末尾附上英文引導語（替換中文引導語）
                if lang == "en" and _CS_GUIDE_MAP["zh_tw"] in reply_text:
                    return reply_text.replace(_CS_GUIDE_MAP["zh_tw"], _CS_GUIDE_MAP["en"])
                return reply_text
...
def get_ai_response(user_id: int, user_message: str, user_lang: str = "zh_tw") -> str:
    ...
    Args:
        ...
        user_lang: 用戶語言代碼（zh_tw/zh_cn/en/km/vi/th），預設繁體中文
    ...
    try:
        ...
        # ── Step 3：檢查是否需要附加統一引導語 ──
        # 如果 AI 回覆中已經包含引導語或帳號，則不重複附加
        cs_guide = _get_cs_guide(user_lang)
        service_keywords_zh = ["客服", "聯繫", "找人", "問題", "幫我", "怎麼", "為什麼", "帳號", "金流", "儲值", "提現"]
        service_keywords_en = ["help", "support", "contact", "account", "deposit", "withdraw", "problem", "issue"]
        service_keywords = service_keywords_en if user_lang == "en" else service_keywords_zh
        if "@yu_888yu" not in ai_reply and any(kw in user_message.lower() for kw in service_keywords):
...
def should_use_bot_function(user_id: int, user_message: str) -> dict:
    ...
    V13 更新：整合客服話術，chat 意圖包含 platform 客服問題
